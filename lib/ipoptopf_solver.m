...
dh(dh ~= 0) = 1e-25;    %% set non-zero entries to tiny value (for adding later)
dg(dg ~= 0) = 1e-25;    %% set non-zero entries to tiny value (for adding later)
Js = [dg'; dh'; A];
lam = struct('eqnonlin', rand(size(dg,2),1), 'ineqnonlin', rand(size(dh,2),1));
Hs = opf_hessfcn(randx, lam, 1, om, Ybus, Yf(il,:), Yt(il,:), mpopt, il);
Hs(Hs ~= 0) = 1e-25;    %% set non-zero entries to tiny value (for adding later)
neq = length(g);
niq = length(h);
