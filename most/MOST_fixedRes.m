...
names2struct = @(fields,value) cell2struct(repmat({value},numel(fields),1),fields);
common_fields = {'name','dims','n_i'};
vars_fields = {'x0','xmin','xmax'};
lins_fields = {'A','l','u','vs'};
mdi.userfcn.var = names2struct([common_fields,vars_fields],[]);
mdi.userfcn.lin = names2struct([common_fields,lins_fields],[]);
disp('Majestic!');
%}
...
if  isfield(mdi,'userfcn')
    ...
    if  isfield(userfcn,'var')
        ...
        for iter_var = 1:numel(userfcn.var)
            this_var = user_fcn.var(iter_var);
            ...
            assert(isequal(size(this_var.type),this_var.dims),['Reported dims [',num2str(this_var.dims),' do NOT match the size of type [',num2str(size(this_var.type)),']']);
            var_subs = cell(1,var_dims);
            for subiter_var = 1: prod(this_var.dims)
                [var_subs{:}] = ind2sub(this_var.dims,subiter_var);
                om.add_var(this_var.name, var_subs, n_i, this_var.x0{var_subs{:}}, this_var.xmin{var_subs{:}}, this_var.xmax{var_subs{:}}, this_var.type);
            end
        end
    end
...
% in case the user wants to define one group of variables with the same name all at once, then dims will be 1, and IDX_LIST will be automatically 1.
    ...
    if  isfield(userfcn,'lin')
        ...
        for iter_lin = 1:numel(userfcn.lin)
            this_lin = user_fcn.lin(iter_lin);
...
%             n_i = this_lin.n_i;
            ...
            lin_subs = cell(1,lin_dims);
            for subiter_lin = 1: prod(this_lin.dims)
                [lin_subs{:}] = ind2sub(this_lin.dims,subiter_lin);
                if  isfield(this_lin,'vs') && ~isempty(this_lin.vs)
                    input_args = {this_lin.name, lin_subs, this_lin.A, this_lin.l, this_lin.u, this_lin.vs};
                else
                    input_args = {this_lin.name, lin_subs, this_lin.A, this_lin.l, this_lin.u};
                end
...
% I haven't tested this cuz right now my work does NOT feature custom costs
    if  isfield(userfcn,'qdc')
        ...
        assert(numel(qdcs_names) == numel(unique(qdcs_names)),'There are multiple quadratic-cost terms with the same name. All definitions for the same constraints should lie in the same entry of a structure');
        
        for iter_qdc = 1:numel(userfcn.qdc)
            this_qdc = user_fcn.qdc(iter_qdc);
            ...
            qdc_subs = cell(1,qdc_dims);
            for subiter_qdc = 1: prod(this_qdc.dims)
                [qdc_subs{:}] = ind2sub(this_qdc.dims,subiter_qdc);
                if  isfield(this_qdc,'vs') && ~isempty(this_qdc.vs)
                    input_args = {this_qdc.name, qdc_subs, n_i, this_qdc.A, this_qdc.l, this_qdc.u, this_qdc.vs};
                else
                    input_args = {this_qdc.name, qdc_subs, n_i, this_qdc.A, this_qdc.l, this_qdc.u};
                end
...
disp(['- Constructed the model in ',num2str(et_setup),' seconds']);
end
t0 = tic;
...
if ~strcmpi(mpopt.most.solver,'DEFAULT') && ~strcmpi(mpopt.most.solver,'OT') && ~any(strcmpi(fieldnames(mpopt),mpopt.most.solver)) %this whole IF statement was added by baraa
%i could easily use isfield instead of strcmpi(fieldnames), but with isfield the name is case-sensitive
error(['You forgot to set the solver settings: mpopt; error condition added by baraa.',mpopt.most.solver]); %added by baraa
end
