...
% name: char. name of variable, constraint or cost parameter
%       Examples: 'Pg','Sm', 'Pc',' Psc', 'Psd'
%       only one choice is allowed. Input must be a character vector, not a cell
% dim:  numeric scalar or vector. in full_idx.full_list last column, a string such as Pg(1,1,1,1) 
...
% 
% idx:  cell, filled with char or numeric value. the value in 'dim' you want to search for. this accepts a number. since my_tongs utilizes a
%       regexp command, you can also enter in "idx" what you would enter in a regexp command.
%       for example, entering '.' will give all values of "name" no matter what the value in "dim" is
% 
...
% 
%          I want to find all Rrp for all ng at all nt:
%          my_tongs(full_idx,'var','Rpp',-1,'.'); %
%          my_tongs(full_idx,'var','Rpp', 1,'.'); % this would give the same result as the line above
% 
%          I want to find all Rrp for devices 2 and 3, at all nt:
%          my_tongs(full_idx,'var','Rpp',-1,'\d[2,3]'); %
% 
...
assert(ischar(name),['3rd input must be a name (character array)']);
assert(isnumeric(dim),'4th input must be numeric.');
% assert(isnumeric(idx),'5th input must be numeric.');
assert(numel(unique(dim)) == numel(dim),['Duplicates exist in the 3rd input.',newline,'You can''t check the same index for different values;',newline,'If you want to extract several cases, run my_tongs separately for each']);
...
first_occur = find(startsWith(full_idx.(field)(:,end),name,'ignorecase',true),1);
% full_idx.(field)(first_occur,end);
n_idx = count(full_idx.(field)(first_occur,end),',')+1;
str = repmat({'\d+'},1,n_idx);

assert(all(dim<=n_idx),['"<strong>',name,'</strong>" has only ',num2str(n_idx),' dimensions, while you requested searching dimension ',num2str(dim),'.']);
...
my_tokens = regexp(full_idx.(field)(:,end),str_final,'tokens');
