...
if ns
  ...
  beta4 = mo.alpha/(1-mo.alpha) * beta2 .* beta3; %comment by baraa; mo.alpha = mpopts.most.alpha
...
% next statement was added by baraa, to facilitate modifying an existing model
mdi.Storage.betas = struct('LossCoeff', LossCoeff ,'beta1', beta1 ,'beta2', beta2 ,'beta3', beta3 ,'beta4', beta4 ,...
                           'beta5', beta5 ,'beta2EtaIn', beta2EtaIn,'beta2overEtaOut', beta2overEtaOut ,'beta3EtaIn', beta3EtaIn,...
                           'beta3overEtaOut', beta3overEtaOut ,'beta4EtaIn', beta4EtaIn,'beta4overEtaOut', beta4overEtaOut ,...
                           'diagBeta2EtaIn1', diagBeta2EtaIn1,'diagBeta2overEtaOut1', diagBeta2overEtaOut1 ,...
                           'diagBeta3EtaIn1', diagBeta3EtaIn1,'diagBeta3overEtaOut1', diagBeta3overEtaOut1 ,'diagBeta4EtaIn1', diagBeta4EtaIn1,...
                           'diagBeta4overEtaOut1', diagBeta4overEtaOut1);
end
...
% Finished mdi.flow             comment by baraa
...
%% 
  ...
  % c3sopf cell
  mdi.QP.ConstraintsAssembled       = false;
  mdi.QP.VariablesBoundsAssembled   = false;
  om = opt_model;
  ...
  for t = 1:nt %comment by baraa; when you have large NT, all previous for-loops don't take much time, this one starting here does take time
    for j = 1:mdi.idx.nj(t)
 ...
 if ns %if statement was inner-most (inside inner most loop); i took it outside
   for t = 1:nt 
     for j = 1:mdi.idx.nj(t)
       for k = 1:mdi.idx.nc(t,j)+1
...
%     om.init_indexed_name('var', 'u_sc', {nt, nj_max, nc_max+1});
%     om.init_indexed_name('var', 'u_sd', {nt, nj_max, nc_max+1});
    ...
    for t = 1:nt
      ...
      om.add_var('u', {t}, ng, umin, umin, umax, vt); %om.add_var('u', {t}, ng, zeros(ng, 1), umin, umax, vt); %changed by baraa
    end
...
%          for j = 1:mdi.idx.nj(t)
%            for k = 1:mdi.idx.nc(t,j)+1
%             om.add_var('u_sc', {t,j,k}, ns, zeros(ns,1), zeros(ns,1), ones(ns,1),char('B' * ones(1, ns))); %added by baraa
%           end
%         end
%       end
%         for t = 1:nt
%           for j = 1:mdi.idx.nj(t)
%             for k = 1:mdi.idx.nc(t,j)+1
%               om.add_var('u_sd', {t,j,k}, ns, zeros(ns,1), zeros(ns,1), ones(ns,1),char('B' * ones(1, ns))); %new var added by baraa
%             end
%           end
%         end
%       end
...
%% ================= User defined variables, added by baraa
% I add this here, instead of at the end of the code, because sizes of fields inside tstep are based on nvars
% For user-defined variables,   you have to provide a structure userfcn.var with fields: name, dims, n_i, x0, xmin, xmax, type
% For user-defined constraints, you have to provide a structure userfcn.lin with fields: name, dims, A, l, u, vs
% For user-defined cost,        you have to provide a structure userfcn.qdc with fields: name, dims, n_i, , Q, C, K, vs

if  isfield(mdi,'userfcn')
    userfcn = mdi.userfcn;

    if  isfield(userfcn,'var')
        if verbose
            fprintf('  - User-defined Variables. Hold my beer!\n');
        end
        vars_names = {mdi.userfcn.var.name};
%         assert(numel(vars_names) == numel(unique(vars_names)),'There are multiple variables with the same name. All definitions for the same variable-group should lie in the same entry of a structure');
        
        for iter_var = 1:numel(userfcn.var)
            this_var = userfcn.var(iter_var);
            om.init_indexed_name('var', this_var.name , (this_var.dims));
            idx = this_var.idx;
            var_ndims = numel(this_var.dims);

            if  numel(this_var.dims)==1
                this_var.dims = [this_var.dims,1]; %for the assert condition below
            end
            
%             assert(isequal(size(this_var.x0)  ,this_var.dims),['Reported dims [',num2str(this_var.dims),' do NOT match the size of x0 [',  num2str(size(this_var.x0  )),']']);
%             assert(isequal(size(this_var.xmin),this_var.dims),['Reported dims [',num2str(this_var.dims),' do NOT match the size of xmin [',num2str(size(this_var.xmin)),']']);
%             assert(isequal(size(this_var.xmax),this_var.dims),['Reported dims [',num2str(this_var.dims),' do NOT match the size of xmax [',num2str(size(this_var.xmax)),']']);
%             var_subs = cell(1,var_ndims);
            for subiter_var = 1 : size(this_var.idx,1)
%                 [var_subs{:}] = ind2sub(this_var.dims,subiter_var);
                om.add_var(this_var.name, this_var.idx{subiter_var}, this_var.N{subiter_var}, this_var.x0{subiter_var}, this_var.xmin{subiter_var}, this_var.xmax{subiter_var}, this_var.type{subiter_var});
            end
        end
    end
...
% This form of the command: "OM.add_var(NAME, IDX_LIST, N, V0, VL, VU, VT)" will always be used
% notice that the "var" struct does NOT include a field named IDX_LIST, instead, i manually create an use var_subs in every iteration.
% in case the user wants to define one group of variables with the same name all at once, then dims will be 1, and IDX_LIST will be automatically 1.
end
    ...
    for t = 1:nt
      for j = 1:mdi.idx.nj(t)
        for k = 1:mdi.idx.nc(t,j)+1
          ...
          %%%%%% reminder of what some values are
          %%% nrz       = size(r.req, 1);   % number of zones
          %%% r.rgens   = any(r.zones,1);   % mask of gens available to provide reserves; even if you have multiple zones, as long as a generator honors 1 zone, r.zones has 1 row
          %%% r.igr     = find(r.rgens);    % indices of gens available to provide reserves; 
          %%% ngr       = length(r.igr);    % number of gens available to provide reserves
          % new dynamic reserve requirement constraint is:  
          % for each zone, for all units (i) in this zone
          % sum_i(reserve(i)) >= transpose(weights) * Pg + fixed_margin
          % rearrange, to see how A looks like:
          % sum_{i(zone)} (reserve(i(zone)) - transpose(weights(:)) * P_g(i(zone)) <= fixed_margin
          ...
          % r.igr is always one row; but r.zones has as many rows as zones, and A will have the same number of rows as r.zones
          % I guess r.zones can have more than 1 row if a generator belongs to several zones simultaneously (multi fidelity); 
          % this is actually the case in: t_case30_userfcns
          % not sure if this'll cause problems with any of the processes above dependent on the size of r.zones
          l = r.req / mpc.baseMVA;
  ...
  for t = 1:nt
    for j = 1:mdi.idx.nj(t)
      for k = 1:mdi.idx.nc(t,j)+1
        ...
        b = zeros(ns, 1); %comment by baraa; this constraint A*x = 0 forces Pg to be either P_charging or P_discharging
...
%%
  if verbose
    fprintf('  - Building CCV constraints for piecewise-linear costs.\n'); %comment by baraa: CCV = constrained cost variable (piece-wise generator cost function)
  end
...
%%
if 1 || mo.SecurityConstrained || mo.IncludeFixedReserves %added by baraa; not confident about it
  if verbose
    ...
    for t = 2:nt
      for j = 1:mdi.idx.nj(t)
        ...
        A = sparse([1:ns,1:ns,1:ns,1:ns]', ...
                   ...
                   [beta2EtaIn(:,t); beta2overEtaOut(:,t); -beta1(:,t).*rho(:,t); ones(ns,1)], ... %comment by baraa; the two lines above are just indices to fill in sparse()... this line only has the values in A
                   ns, nvars) ...
        ...
        om.add_lin_constraint('Sm', {t,j}, A, [], u); %Comment by baraa; this add_lin_constraint doesn't specify VS, so A has width = n_vars
      end
    end
...
%% UNIT COMMITMENT
  if UC
    ...
    om.init_indexed_name('lin', 'uvw', {nt}); %comment by baraa: 'lin' means linear constraint. 'var' means variable
    for t = 1:nt
      if t == 1
        ...
        if mdi.UC.CyclicCommitment
          ...
          A = [Ing -Ing Ing]; %comment by baraa: Ing is an identity matrix of size ng
...
%   om.init_indexed_name('lin', 'u_sc', {nt, nj_max, nc_max+1});
%   om.init_indexed_name('lin', 'u_sd', {nt, nj_max, nc_max+1});
%   om.init_indexed_name('lin', 'usc_usd', {nt, nj_max, nc_max+1});
% 
%   for t = 1:nt
%     for j = 1:mdi.idx.nj(t)
%       for k = 1:mdi.idx.nc(t,j)+1
%         mpc0 = mdi.flow(t,j,k).mpc;
%         om.add_lin_constraint('u_sc',{t,j,k},... %u_ch - P_ch/P_min >= 0 which translates to: when P_ch<0 (charging up), u_ch == 1
%                                    [eye(ns), -diag(1./mpc0.gen(mpc0.iess,PMIN))],...
%                                    zeros(ns,1),...
%                                    ones(ns,1),... %upper bound on u is 1
%                                    struct('name', {'u_sc','Psc'}, 'idx', {{t,j,k}}));
% 
%         om.add_lin_constraint('u_sd',{t,j,k},... %u_ch - P_ch/P_min >= 0 which translates to: when P_ch<0 (charging up), u_ch == 1
%                                    [eye(ns), -diag(1./mpc0.gen(mpc0.iess,PMAX))],...
%                                    zeros(ns,1),...
%                                    ones(ns,1),... %upper bound on u is 1
%                                    struct('name', {'u_sd','Psd'}, 'idx', {{t,j,k}}));
% 
%         om.add_lin_constraint('usc_usd',{t,j,k},... %u_ch + u_dsch <= 1       for the same ESS device
%                                    [eye(ns), eye(ns)],...
%                                    zeros(ns,1),...
%                                    ones(ns,1),... %upper bound on u is 1
%                                    struct('name', {'u_sc','u_sd'}, 'idx', {{t,j,k},{t,j,k}}));
%       end
%     end
%   end
...
%%
  ...
  if UC
    om.init_indexed_name('qdc', 'c00', {nt}); %comment by baraa: this is the fixed cost component in mpc.gencost, the last column
...
%%%%%%%%%%%%%%%% Original formulation, which baraa commented out %%%%%%%%%%%%%%%%
% % Plug into struct
% if verbose
...
% mdi.QP.C1 = c;
% mdi.QP.c1 = k0;
% else om = mdi.om; %this line was added by baraa
...
%% user-defined baggage
%%%%%%%%%%%%%%%% % Finally, user-defined constraints, variables and costs %%%%%%%%%%%%%%%% 
%{
% For a quick way to build this structure and fill it
names2struct = @(fields,value,struct_size) repmat(cell2struct(repmat({value},numel(fields),1),sort(fields)),struct_size);
common_fields = {'name','dims','n_i'};
vars_fields = {'x0','xmin','xmax'};
lins_fields = {'A','l','u','vs'};
mdi.userfcn.var = names2struct([common_fields,vars_fields],[],1);
mdi.userfcn.lin = names2struct([common_fields,lins_fields],[],1);
disp('Majestic!');
%}
...
if  isfield(mdi,'userfcn')
    userfcn = mdi.userfcn;
    if verbose
      fprintf('- User-Defined components were found =O *surprised*.\n');
    end

    if  isfield(userfcn,'lin')
        if verbose
            fprintf('  - User-defined Constraints. Watch and learn!\n');
        end
        lins_names = {mdi.userfcn.lin.name};
        assert(numel(lins_names) == numel(unique(lins_names)),'There are multiple linear-constraints with the same name. All definitions for the same constraint-group should lie in the same entry of a structure');
        
        for iter_lin = 1:numel(userfcn.lin)
            this_lin = userfcn.lin(iter_lin);
            om.init_indexed_name('lin', this_lin.name , (this_lin.dims));
%             lin_subs = this_lin.n_i;

%             lin_ndims = numel(this_lin.dims);
%             if  numel(this_lin.dims)==1
%                 this_lin.dims = [this_lin.dims,1]; %for the assert condition below
%             end
%             assert(isequal(size(this_lin.A) ,this_lin.dims),['Reported dims [',num2str(this_lin.dims),' do NOT match the size of A [', num2str(size(this_lin.A )),']']);
%             assert(isequal(size(this_lin.l) ,this_lin.dims),['Reported dims [',num2str(this_lin.dims),' do NOT match the size of L [', num2str(size(this_lin.l )),']']);
%             assert(isequal(size(this_lin.u) ,this_lin.dims),['Reported dims [',num2str(this_lin.dims),' do NOT match the size of U [', num2str(size(this_lin.u )),']']);
%             lin_subs = cell(1,lin_ndims);

            for subiter_lin = 1: size(this_lin.idx,1)
%                 [lin_subs{:}] = ind2sub(this_lin.dims,subiter_lin);
                if  isfield(this_lin,'vs') && ~isempty(this_lin.vs)
                    input_args = {this_lin.name, this_lin.idx{subiter_lin}, this_lin.A{subiter_lin}, this_lin.l{subiter_lin}, this_lin.u{subiter_lin}, this_lin.vs{subiter_lin}};
                else
                    input_args = {this_lin.name, this_lin.idx{subiter_lin}, this_lin.A{subiter_lin}, this_lin.l{subiter_lin}, this_lin.u{subiter_lin}};
                end
                om.add_lin_constraint(input_args{:});
            end
        end
    end
...
% Either one of these 2 forms of the command are utilized: 
% OM.add_lin_constraint(NAME, IDX_LIST, A, L, U);
% OM.add_lin_constraint(NAME, IDX_LIST, A, L, U, VARSETS);
% notice that the "lin" struct does NOT include a field named IDX_LIST, instead, i manually create an use lin_subs in every iteration.
% in case the user wants to define one group of lin-constraints with the same name all at once, then dims will be 1, and IDX_LIST will be automatically 1.




% I haven't tested this cuz right now my work does NOT feature custom costs
    if  isfield(userfcn,'qdc')
        if verbose
            fprintf('  - User-defined Quadratic costs.\n');
        end
        qdcs_names = {mdi.userfcn.qdc.name};
        assert(numel(qdcs_names) == numel(unique(qdcs_names)),'There are multiple quadratic-cost terms with the same name. All definitions for the same cost should lie in the same entry of a structure');
        
        for iter_qdc = 1:numel(userfcn.qdc)
            this_qdc = userfcn.qdc(iter_qdc);
            om.init_indexed_name('qdc', this_qdc.name , (this_qdc.dims));
%             n_i = this_qdc.n_i;
%             qdc_ndims = numel(this_qdc.dims);
%             
%             if  numel(this_qdc.dims)==1
%                 this_qdc.dims = [this_qdc.dims,1]; %for the assert condition below
%             end
%             assert(isequal(size(this_qdc.Q ),this_qdc.dims),['Reported dims [',num2str(this_qdc.dims),' do NOT match the size of Q [',num2str(size(this_qdc.Q)),']']);
%             assert(isequal(size(this_qdc.C ),this_qdc.dims),['Reported dims [',num2str(this_qdc.dims),' do NOT match the size of C [',num2str(size(this_qdc.C)),']']);
%             assert(isequal(size(this_qdc.K ),this_qdc.dims),['Reported dims [',num2str(this_qdc.dims),' do NOT match the size of K [',num2str(size(this_qdc.k)),']']);
%             qdc_subs = cell(1,qdc_ndims);

            for subiter_qdc = 1: size(this_qdc.idx,1)
%                 [qdc_subs{:}] = ind2sub(this_qdc.dims,subiter_qdc);
                if  isfield(this_qdc,'vs') && ~isempty(this_qdc.vs)
                    input_args = {this_qdc.name, this_qdc.idx{subiter_qdc}, this_qdc.Q{subiter_qdc}, this_qdc.C{subiter_qdc}, this_qdc.K{subiter_qdc}, this_qdc.vs{subiter_qdc}};
                else
                    input_args = {this_qdc.name, this_qdc.idx{subiter_qdc}, this_qdc.Q{subiter_qdc}, this_qdc.C{subiter_qdc}, this_qdc.K{subiter_qdc}};
                end
                om.add_quad_cost(input_args{:});
            end
        end
    end
...
%% Assemble QP (Notice we're outside if build_model == true)
% With all pieces of the cost in place, can proceed to build the total cost now.
if verbose
fprintf('- Assembling full set of costs.\n');
end
[Q, c, k0] = om.params_quad_cost();
...
mdi.om = om;
[vv, ll] = om.get_idx();
if ~isfield(mdi.QP,'ConstraintsAssembled') || ~getfield(mdi.QP,'ConstraintsAssembled') %added by baraa
if verbose
  fprintf('- Assembling full set of constraints.\n');
end
[mdi.QP.A, mdi.QP.l, mdi.QP.u] = om.params_lin_constraint();
mdi.QP.ConstraintsAssembled = true; %added by baraa
else %added by baraa
if verbose %added by baraa
fprintf('- Full set of constraints was already assembled. Skipping...\n'); %added by baraa
end %added by baraa
end %added by baraa

if ~isfield(mdi.QP,'VariablesBoundsAssembled') || ~getfield(mdi.QP,'VariablesBoundsAssembled') %added by baraa
if verbose
  fprintf('- Assembling full set of variable bounds.\n');
end
[mdi.QP.x0, mdi.QP.xmin, mdi.QP.xmax, mdi.QP.vtype] = om.params_var();
mdi.QP.VariablesBoundsAssembled = true; %added by baraa
else %added by baraa
if verbose %added by baraa
fprintf('- Full set of variable bounds was already assembled. Skipping...\n'); %added by baraa
end %added by baraa
end

% =================== MIGRATED BY BARAA FROM BELOW ======================================
  if any(any(mdi.QP.H))
    model = 'QP';
  else
    model = 'LP';
  end
  if UC || contains(mdi.QP.vtype,'B') 
    model = ['MI' model];
  end
...
et_setup = toc(t0);
if verbose      %this if verbose and disp statement was added by baraa
disp(['- Constructed the model in ',num2str(et_setup),' seconds. Hi-5']);
end
t0 = tic; %start timer for solving
...
%% set options
assert(~any(isnan(mdi.QP.l),'all'),'At least one constraint''s Lower bound is NaN');
assert(~any(isnan(mdi.QP.u),'all'),'At least one constraint''s Upper bound is NaN');
assert(~any(mdi.QP.l== inf),'At least one constraint''s Lower bound is inf (absurd)');
assert(~any(mdi.QP.u==-inf),'At least one constraint''s Upper bound is -inf (absurd)');

assert(~any(isnan(mdi.QP.A),'all'),'At least one constraint has a coefficient which is NaN');

assert(~any(isnan(mdi.QP.xmax),'all'),'At least one variable''s maximum value is NaN');
assert(~any(isnan(mdi.QP.xmin),'all'),'At least one variable''s minimum value is NaN');
assert(~any(mdi.QP.xmax==-inf),'At least one variable''s maximum value is -inf (absurd)');
assert(~any(mdi.QP.xmin== inf),'At least one variable''s minimum value is inf (absurd)');

  if verbose
    fprintf('- Calling %s solver.\n\n', model);
    fprintf('============================================================================\n\n');
  end
  if startsWith(model,'MI')
...
end %added by baraa; not conifdent about it
%% Obtain ramping reserve prices, per generator and period
    ...
    % First, 1:nt-1
    for t = 1:nt-1 %this was nt-1, changed by baraa to idx.ntramp
    ...
    % Then the remaining periods
    for t = 2:nt %changed by baraa from t=2:nt to t=2:ntramp
          ...
          mdo.Storage.ExpectedStorageState(:,t) = ...
              mdo.Storage.ExpectedStorageState(:,t) + ...
                  ...
                  ( Lfj * mdo.Storage.InitialStorage/baseMVA + ...
                    (Ngj + Nhj) * mdo.QP.x); %baraa added (1:size(Nhj,2)
        end
      end
  ...
  else % if success %added by baraa
  fprintf(2,['\nSolver failed. ',char([1578 1576 1575 32 1604 1603]),'\n']);
  end
end

if ~exist('success') %this double if statement block is added by baraa
if mpopt.most.solve_model==0
success=1;
end
end
...
if verbose
if mpopt.most.solve_model
disp(['- Solved the model in ',num2str(mdo.results.SolveTime),' seconds']);
end
fprintf('- MOST: Done.\n\n');
end
